 ...
 */
+ (instancetype)meTaskWithCompletionHandler:(KOSessionTaskCompletionHandler)completionHandler DEPRECATED_MSG_ATTRIBUTE("전화번호 로그인이 오픈되면서 이메일이 없는 계정이 존재할 수 있습니다. 다양한 상황에 대처할 수 있는 /v2/user/me가 적용된 새로운 API를 사용해주세요.");
...
+ (instancetype)meTaskWithSecureResource:(BOOL)secureResource
                       completionHandler:(KOSessionTaskCompletionHandler)completionHandler DEPRECATED_MSG_ATTRIBUTE("전화번호 로그인이 오픈되면서 이메일이 없는 계정이 존재할 수 있습니다. 다양한 상황에 대처할 수 있는 /v2/user/me가 적용된 새로운 API를 사용해주세요.");
